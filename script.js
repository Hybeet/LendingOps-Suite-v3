...
const API_CONFIG = Object.freeze({
    ...
    BUSINESS_PARSER:
        "https://script.google.com/macros/s/AKfycbxNPVK7PHGOAEPxBt-cqFAbxYXFhyuuYHcRDsrBA4mnYoEucGMJBZVl1UJByLEuyPcR/exec",
...
async function runCalculation() {
    ...
    if (
        ...
        const sheetCorrectionOverride =
            $("sheet-correction-override");


        if (
            sheetCorrectionOverride
        ) {

            sheetCorrectionOverride.checked =
                false;
...
async function syncParserMetadataToDataCoreEnv(
    marketName,
    reportDate,
    rawReportText
) {
    ...
    if (syncButton) {
        ...
        syncButton.title =
            "Waiting for DataCore customer population.";
    ...
    if (
        ...
        if (refreshButton) {
            ...
            refreshButton.title =
                "Extract a report before refreshing DataCore.";
    ...
    if (refreshButton) {
        ...
        refreshButton.title =
            "Refresh DataCore using the current report.";
    ...
    /*
     * ---------------------------------------------------------------
     * AUTHORITATIVE DATACORE LOAD PROMISE
     * ---------------------------------------------------------------
     */
...
async function refreshDataCore() {
    ...
    /*
     * ---------------------------------------------------------
     * READ CURRENT REPORT INPUT
     * ---------------------------------------------------------
     *
     * IMPORTANT:
     * We deliberately read reportInput NOW.
     *
     * This means the officer can correct the pasted report
     * without running the full Extract Data process again.
     * ---------------------------------------------------------
     */
    ...
    const rawReportText =
        reportInput &&
        typeof reportInput.value === "string"
    ...
    const market =
        marketInput &&
        typeof marketInput.value === "string"
    ...
    if (!market) {
        ...
        setText(
            "datacore-status-subtext",
            "Market is missing. Extract the report first."
        );
    ...
    if (!date) {
        ...
        setText(
            "datacore-status-subtext",
            "Report date is missing. Extract the report first."
        );
    ...
    setText(
        "datacore-status-subtext",
        `Refreshing DataCore — ${market} — ${date}`
    );


    /*
     * ---------------------------------------------------------
     * RESET CURRENT DATACORE DISPLAY
     * ---------------------------------------------------------
     */
    ...
    if (gridBody) {
        ...
        gridBody.innerHTML = `
            <tr>
                ...
                >
                    Refreshing DataCore customer records...
                </td>
            </tr>
        `;
    ...
    /*
     * ---------------------------------------------------------
     * START A COMPLETELY NEW DATACORE LOAD
     * ---------------------------------------------------------
     *
     * We reuse the existing authoritative loader.
     *
     * This does NOT call extractData().
     * ---------------------------------------------------------
     */
    ...
    try {
        ...
        /*
         * -----------------------------------------------------
         * WAIT FOR THE NEW CUSTOMER LOAD
         * -----------------------------------------------------
         */

        const records =
            await waitForDataCoreCustomerPopulation();
        ...
        if (
            ...
            throw new Error(
                "DataCore returned an invalid customer record list."
            );
        ...
        setText(
            "datacore-status-subtext",
            `DataCore refreshed — ${market} — ${date}`
        );
        ...
        /*
         * Let the existing DataCore error UI remain
         * responsible for displaying detailed errors where
         * applicable.
         */
...
async function waitForDataCoreCustomerPopulation() {

    /*
     * ---------------------------------------------------------
     * WAIT FOR DATACORE TO START
     *
     * The Disbursement watcher can detect the report before
     * script.js reaches syncParserMetadataToDataCoreEnv().
     *
     * Therefore we MUST NOT throw immediately.
     * ---------------------------------------------------------
     */
    ...
    while (
        ...
        if (
            ...
            throw new Error(
                "DataCore customer population did not start within the expected time."
            );
        }
    ...
    /*
     * ---------------------------------------------------------
     * DATACORE HAS STARTED.
     *
     * NOW WAIT FOR THE ACTUAL CUSTOMER POPULATION REQUEST.
     * ---------------------------------------------------------
     */
    ...
    if (
        ...
        throw new Error(
            "DataCore customer population started, but no load request is available."
        );
    ...
    const records =
        await dataCoreCustomerLoadPromise;
    ...
    if (
        ...
        throw new Error(
            "DataCore customer population completed with an invalid record list."
        );
...
window.waitForDataCoreCustomerPopulation =
    waitForDataCoreCustomerPopulation;
...
async function fetchActiveMarketRowsFromSheets() {
    ...
    try {
       ...
       /* console.log(
        ...
        return appState.dataCore.loadedRecords;
...
function clearWorkspace() {
    ...
    /*
     * ---------------------------------------------------------------
     * CLEAR HISTORICAL VALIDATION AUDIT UI
     * ---------------------------------------------------------------
     *
     * The audit state above is reset, but the previous audit can
     * still remain visible on screen unless its DOM elements are
     * explicitly cleared and hidden.
     * ---------------------------------------------------------------
     */

    const historicalAuditBox =
        $("historical-audit-box");


    const historicalAuditDetails =
        $("historical-audit-details");
    ...
    if (
        ...
        historicalAuditBox.classList.add(
            "hidden"
        );

        historicalAuditBox.classList.remove(
            "has-error"
        );
    ...
    if (
        historicalAuditDetails
    ) {

        historicalAuditDetails.innerHTML =
            "";
...
function addActionButtonClickEffect(
    button,
    originalText,
    workingText
) {
    ...
    /*
     * Prevent this helper from being attached more than once.
     */

    if (
        button.dataset.actionEffectAttached ===
        "true"
    ) {
        return;
    }
    ...
    button.addEventListener(
        ...
        function() {
            ...
            /*
             * Use the existing button text while temporarily
             * showing that the click was received.
             */
            ...
            /*
             * ----------------------------------------------------
             * RELEASE THE PRESSED VISUAL STATE
             * ----------------------------------------------------
             */
...
document.addEventListener(
    ...
    () => {
        ...
        const refreshDataCoreButton =
            $("refresh-datacore-btn");

        refreshDataCoreButton?.addEventListener(
            "click",
            refreshDataCore
        );
         ...
         * =========================================================
         *
         * We deliberately leave Clear Workspace unchanged.
         * =========================================================
         */
